...
const RestService = {
  ...
  applyJob: (jobId,userId) => AxiosService.post(GLOBELCONSTANT.JOB.APPLY_JOB.replace(":jobId", jobId).replace(":userId", userId)),
  ...
  getJobAppliedByUsers: (userId) => AxiosService.get(GLOBELCONSTANT.JOB.JOB_APPLIED_BY_USERS.replace(":userId", userId)),
